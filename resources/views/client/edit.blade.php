    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="/user/clients/{{ $client->id }}" method="POST">
                        @method('PUT')
                        @csrf
                        @php
                            $detail = $client->userMeta;
                        @endphp
                        <input type="hidden" name="district_name" value="{{ $detail->district_name }}">
                        <input type="hidden" name="ward_name" value="{{ $detail->ward_name }}">
                        <div class="row">
                            <div class="col-12 col-lg-4">
                                <div class="form-group">
                                    <label>ID X Media</label>
                                    <input type="text" class="form-control @error('xmedia_id') is-invalid @enderror"
                                        name="xmedia_id" value="{{ $client->userMeta->xmedia_id }}">
                                    @error('xmedia_id')
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('nik') is-invalid @enderror"
                                        name="nik" value="{{ $client->userMeta->nik }}">
                                    @error('nik')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('phone') is-invalid @enderror"
                                        name="phone" value="{{ $client->userMeta->phone }}">
                                    @error('phone')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-control @error('package_id') is-invalid @enderror"
                                        ...
                                        @foreach ($packages as $item)
                                            <option value="{{ $item->id }}"
                                                {{ ($item->id == $client->userMeta->package_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror status-select"
                                        ...
                                        <option {{ $client->installations->status == 'Aktif' ? 'selected' : '' }}
                                            value="Aktif">Aktif</option>
                                        <option {{ $client->installations->status == 'Antrian' ? 'selected' : '' }}
                                            value="Antrian">Belum Aktif</option>
                                    </select>
                                    @error('status')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                    @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="district" class="form-control @error('district_id') is-invalid @enderror"
                                        ...
                                        @foreach ($district as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id == $detail->district_id ? 'selected' : '' }}>
                                                {{ $item->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="form-group">
                                    ...
                                    <select id="ward" class="form-control @error('ward_id') is-invalid @enderror"
                                        name="ward_id">
                                        <option value="{{ $detail->ward_id }}">{{ $detail->ward_name }}
                                        </option>
                                    </select>
                                    @error('ward_id')
                                        <small class="form-text text-danger">{{ $message }}</small>
                            ...
                            <div class="col-12 col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control @error('end_date') is-invalid @enderror"
                                        name="end_date" value="{{ explode(' ', $client->installations->end_date)[0] }}">
                                    @error('end_date')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group">
                            <label>Alamat</label>
                            <textarea class="form-control @error('address') is-invalid @enderror" name="address" cols="30" rows="4">{{ $detail->address }}</textarea>
                            @error('address')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
